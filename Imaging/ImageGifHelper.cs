...
//https://github.com/pgodwin/OcfLzw

//Thumbnail
//https://learn.microsoft.com/en-us/dotnet/api/system.windows.media.imaging.gifbitmapencoder?view=windowsdesktop-8.0

namespace Imaging
{
    public class ImageGifHelper
    {
        private Dictionary<Pixel, int> _pixelToColorTableMap;

        private List<Frame> Frames { get; set; }

        private Dictionary<string, Pixel> ColorTable { get; set; }

        /// <summary>
        ///     Creates and writes .GIF-encoded data to a given file.
        /// </summary>
        public void CreateGIF(string FileOutputPath, bool ShouldRepeat, int width, int height)
        {
            ...
            // Create the .GIF
            Trace.WriteLine($"Creating a new .GIF at the following path: {FileOutputPath}.");

            using (var fileStream = new FileStream(FileOutputPath, FileMode.Create))
        ...
        private void WriteHeader(BinaryWriter output)
        {
            ...
            output.Write(new[] {'G', 'I', 'F'});
            output.Write(new[] {'8', '9', 'a'});
        }
        ...
        /// </summary>
        /// <param name="output"></param>
        private void WriteLogicalScreenDescriptor(BinaryWriter output, int Width, int Height)
        {
        ...
        private BitArray LogicalScreenDescriptorGlobalColorTableSize(int? size = null)
        {
            ...
            // log2(total colors) - 1
            ret = (byte)Math.Ceiling(Math.Log(total, 2.0) - 1.0);
        ...
        private void WriteGlobalColorTable(BinaryWriter output)
        {
            ...
            var placeholder = new Pixel {Red = 0, Green = 0, Blue = 0};
        ...
        private string CreateLZWEncodedImageData(List<Pixel> imageData)
        {
            ...
            var dictionary = LZWEncodedImageDataDictionary();
            var codeSize =
                GetLZWStartingCodeSize(dictionary
                    .Count); // In bits; this value will grow as we enter in more codes in our code table
            ...
            var clear = dictionary.Count - 2; // Holds the index for the clear code
            var eoi = dictionary.Count - 1; // Holds the index for the end-of-information code

            w = _pixelToColorTableMap[imageData[0]].ToString();
            encodedImageData =
                ConvertToBinaryString(clear, codeSize); // First piece of encoded data must be the clear code

            for (var l = 1; l < imageData.Count; l++)
            {
                k = _pixelToColorTableMap[imageData[l]];

                var wc = w + k; // Build an "index buffer" that equals w + k
        ...
        private double GetLZWStartingCodeSize(int imageDataDictionaryCount)
        {
            ...
            var GCTS = LogicalScreenDescriptorGlobalColorTableSize(imageDataDictionaryCount);
            var bytes = new byte[1];
            GCTS.CopyTo(bytes, 0);
        ...
        private Dictionary<string, int> LZWEncodedImageDataDictionary()
        {
            ...
            var sizeOfColorTable = ColorTable.Count;
            var dictionary = new Dictionary<string, int>();

            // Write all color codes to the dictionary
            for (var i = 0; i < sizeOfColorTable; i++)
            {
            ...
            for (var i = sizeOfColorTable; i < (int)Math.Pow(2.0, (bytes[0] + 1)); i++)
            {
                dictionary.Add(i.ToString(), i);
            }
        ...
        private void WriteLZWEncodedImageData(BinaryWriter output, string LZWEncodedImageData)
        {
            ...
            var bytes = GetLZWEncodedImageDataBytes(LZWEncodedImageData);

            var lzwMinimumCodeSize = new byte[1];
            ...
            var remainingBytes = GetRemainingBytesInSubBlock(bytes, 0);
            var writeByteLength = true;
        ...
        private byte[] GetLZWEncodedImageDataBytes(string LZWEncodedImageData)
        {
            ...
            var returnMe = new List<byte>();
            var pos = LZWEncodedImageData.Length - 1;
